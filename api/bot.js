...
// Получаем ID группы обсуждений и канала из переменных окружения
const DISCUSSION_GROUP_ID = process.env.DISCUSSION_GROUP_ID;
const CHANNEL_ID = process.env.CHANNEL_ID || -1002696885166; // ID канала "Я Спектр ♦️"
...
function getWelcomeMessage(userId, firstName) {
  if (userId === KNOWN_USERS.SPECTR) {
    ...
    return `👋 Привет${firstName ? ', ' + firstName : ''}! Я — автоматический бот для канала.\n\nМоя задача — добавлять комментарии с правилами под каждым постом в канале.\n\nПодписывайся на канал, чтобы видеть меня в действии! 😊`;
  }
}
    ...
    if (update.message && DISCUSSION_GROUP_ID) {
      ...
      if (chatId.toString() === DISCUSSION_GROUP_ID.toString()) {
        ...
        if (message.forward_from_chat) {
          console.log('Message is forwarded from chat ID:', message.forward_from_chat.id);
          
          // Проверяем по ID канала, а не по username
          if (message.forward_from_chat.id.toString() === CHANNEL_ID.toString()) {
            console.log('New post from target channel detected in discussion group');
            ...
            console.log('Message is forwarded from different channel ID:', message.forward_from_chat.id);
          }
        } else {
